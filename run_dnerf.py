...
def train():
    ...
    #Rotate camera for rendering
    for i in range(render_poses.shape[0]):
        rotation_y = np.array([[np.cos(-i*np.pi/720), 0, np.sin(-i*np.pi/720)],
                         [0, 1, 0], 
                         [-np.sin(-i*np.pi/720), 0, np.cos(-i*np.pi/720)]])
        rotation_x = np.array([[1, 0, 0],
                         [0, np.cos(i*np.pi/720), -np.sin(i*np.pi/720)], 
                         [0, np.sin(i*np.pi/720), np.cos(i*np.pi/720)]])
        render_poses[i,:,:3] =  np.dot(rotation_y,np.dot(rotation_x, render_poses[i,:,:3]))
        #render_poses[i,2,3] =  render_poses[i,2,3] + (0.005 * i)
